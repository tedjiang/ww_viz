...
ww_palette <- c("#C84900", "#709896", "#27647B")
...
bar_saEcon_ap <- ggplot(sa_econ_ap, aes(x = feedstock, y = ap, group = notes))+ 
  geom_bar(stat = "identity", aes(fill = notes), position = "dodge")+
  theme_minimal()+
  labs(x = "Feedstock", y = expression(~Acidification~Potential(~kg-SO^2~equivalent)), fill = "Price Scenario")+
  scale_fill_manual(breaks = c("High Bound","Baseline","Low Bound"), values = ww_palette)+
  coord_flip()
...
bar_saEcon_ep <- ggplot(sa_econ_ep, aes(x = feedstock, y = ep, group = notes))+ 
  geom_bar(stat = "identity", aes(fill = notes), position = "dodge")+
  theme_minimal()+
  labs(x = "Feedstock", y = expression(~Eutrophication~Potential~kg-PO^4~equivalent), fill = "Price Scenario")+
  scale_fill_manual(breaks = c("High Bound","Baseline","Low Bound"), values = ww_palette)+
  coord_flip()+
  guides(fill = FALSE)
...
bar_saEcon_ap_coff <- ggplot(sa_econ_ap_coff, aes(x = feedstock, y = ap, group = notes))+ 
  geom_bar(stat = "identity", aes(fill = notes), position = "dodge")+
  theme_minimal()+
  labs(x = "Feedstock", y = expression(~Aciditication~Potential~(kg-SO^2~equivalent)), fill = "Price Scenario")+
  scale_fill_manual(breaks = c("High Bound","Baseline","Low Bound"), values = ww_palette)+
  coord_flip()
